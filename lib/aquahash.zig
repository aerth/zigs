...
//
// same argon2, but.. allow zero salt and lower mem for fasthash
...
pub fn kdf(
    ...
    if (salt.len > max_int) return KdfError.WeakParameters;
...
const PhcFormatHasher = struct {
    ...
    pub fn create(
        allocator: mem.Allocator,
        password: []const u8,
        salt_length: usize, // randomly generated salt
        params: Params,
        mode: Mode,
        buf: []u8,
    ) HasherError![]const u8 {
        if (params.secret != null or params.ad != null) return HasherError.InvalidEncoding;
        if (salt_length > max_salt_len) {
            return Error.WeakParameters;
        }
        var hash: [default_hash_len]u8 = undefined;
        var saltbuf: [1024]u8 = undefined;
        if (salt_length != 0) {
            crypto.random.bytes(&saltbuf); // reads 1024 random bytes anyway
        }
        const salt = saltbuf[0..salt_length];
...
pub fn strHash(
    password: []const u8,
    salt_length: u8, // for randomly generated salt
    options: HashOptions,
    out: []u8,
) Error![]const u8 {
...
// test output of shell cmd:
//   printf 'aquachain' | aquahash "" -k 1 -t 1 -id -l 32 -r
//

test "aquahash" {
    const allocator = std.testing.allocator;
    const input = [_]u8{ 0x61, 0x71, 0x75, 0x61, 0x63, 0x68, 0x61, 0x69, 0x6e };
    const salt: [0]u8 = undefined; // no salt
    var out: [32]u8 = undefined;
    const aquahash = HashOptions{
        .allocator = allocator,
        .params = .{ .t = 1, .m = 1, .p = 1 }, // argon2id(1,1,1)
        .mode = Mode.argon2id,
    };
    try kdf(allocator, &out, &input, &salt, aquahash.params, aquahash.mode);
    var hexed: [64 + 2]u8 = undefined;
    _ = try std.fmt.bufPrint(&hexed, "0x{}", .{std.fmt.fmtSliceHexLower(out[0..])});
    try std.testing.expect(std.mem.eql(u8, &hexed, "0x1090f3198e771bbe9b3fc9f1291ce5a4744d796e0f099aa02cb681ede75209c0"));
}
test "argon2d" {
...
test "phc format hasher" {
    ...
    inline for (0..65) |saltlen| {
        ...
        // std.debug.print("hash: {s} ({d} bytes, saltlen={d})\n", .{ hash, hash.len, saltlen });
    }
}
