//! modified to use zigs.networking timeout
//!
//!
//!
//!
...
/// This is the timeout in seconds that will be reset before every read/write, set to each connection
timeout: ?u32 = null,
...
pub const Connection = struct {
    ...
    pub fn readvDirectTls(conn: *Connection, buffers: []std.os.iovec) ReadError!usize {
        if (conn.timeout != null) {
            networking.set_stream_timeout(conn.stream.handle, conn.timeout) catch return error.UnexpectedReadFailure;
        }
    ...
    pub fn writeAllDirectTls(conn: *Connection, buffer: []const u8) WriteError!void {
        networking.set_stream_timeout(conn.stream.handle, conn.timeout) catch return error.UnexpectedWriteFailure;
    ...
    pub fn close(conn: *Connection, allocator: Allocator) void {
        if (conn.timeout != null) {
            networking.set_stream_timeout(conn.stream.handle, conn.timeout) catch {};
        }
        if (conn.protocol == .tls) {
            if (disable_tls) unreachable;
...
pub fn connectTcp(client: *Client, host: []const u8, port: u16, protocol: Connection.Protocol) ConnectTcpError!*Connection {
    if (client.connection_pool.findConnection(.{
        ...
        networking.set_stream_timeout(node.stream.handle, client.timeout) catch return error.UnexpectedConnectFailure;
        node.timeout = client.timeout;
        return node;
    }
    ...
    const stream = networking.tcpConnectToHost(client.allocator, host, port) catch |err| switch (err) {
